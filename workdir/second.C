...
#define TRIGGERDELAY 1200 //ns shift of drift times  // in "go_settings.h"
...
//          totCh2 = MakeH1("totCh2","totCh2", 20000, -200, 200, "ns");
       ...
       channelHits = MakeH1("channelHits","channelHits", 4000, 0,400, "15*TDC + channel");
        ...
        meta_potato_h = MakeH2("meta_potato","meta_potato",500,t1_L,t1_R,tot_R, tot_L, tot_R, "t1 (ns);tot (ns)");
        meta_t2_t1_h = MakeH2("meta_t2_t1","meta_t2_t1",2000,t1_L,t1_R,1000, t1_L, t1_R, "t1 (ns);t2 (ns)");
        Int_t nchan_coinc = 120;
...
//                  if(t1_vs_ref < -200 && tot[i]*1e9 > 0 )	FillH1(t1_h[i],t1_vs_ref ); // with noise rejecting cuts
		// if(  tot[i]*1e9 > 50 ) 	FillH1(t1_h[/home/hadaq/Downloads/cts-dumpi],t1_vs_ref ); // with noise rejecting cuts
...
class SecondProcFish : public base::EventProc {
   protected:
...
//       int evt_no;
   ...
   public:
      SecondProcFish(const char* procname, const char* _tdcid1, const char* _tdcid2) :
         base::EventProc(procname),
         fTdcId1(_tdcid1),
         fTdcId2(_tdcid2),
         hNumHits(0)
      {
         printf("Create %s for %s, %s\n", GetName(), fTdcId1.c_str(),  fTdcId2.c_str() );
        ...
        coinc_matrix = MakeH2("coinc_matrix","coinc_matrix", nchan_coinc, 0.5, 0.5+nchan_coinc,  nchan_coinc, 0.5, 0.5+nchan_coinc,"channel layer2; channel layer 3" );
        meta_fish =  MakeH2("meta_fish","meta_fish", 500, -200, +300, 200, -100, 100, "t1a + t1b (ns); t1a - t1b  (ns)");
        potato_fish_h = MakeH2("potato_fish","potato_fish",1000,-400, +600,1000, 0, 1000, "t1 (ns);tot (ns)");
        fish_width_h =  MakeH1("fish_width","fish_width", 200, -200, +300,  "t1a + t1b (ns) ");
        fish_width_h2 =  new TH1F("fish_width2","fish_width2", 200, -200, +300 );
        fish_width_h2->SetDirectory(0);
        RegisterObject(fish_width_h2);
        
        //~ for( unsigned i=0; i<10; i++ ) {
			           
           //~ fish[i] = MakeH1(chno,chno,  500, -200, +300, 200, -100, 100, "t1a + t1b (ns); t1a - t1b  (ns)");
        //~ }
         SetStoreEnabled();
      }
...
//          printf("### DEBUG ###\n");
       //  for (unsigned n=0;n<8;n++) fHits[n] = 0.;
          ...
          hadaq::TdcSubEvent* subLay2 =
               dynamic_cast<hadaq::TdcSubEvent*> (ev->GetSubEvent(fTdcId1));
          hadaq::TdcSubEvent* subLay3 =
               dynamic_cast<hadaq::TdcSubEvent*> (ev->GetSubEvent(fTdcId2));                      
         // hadaq::TdcSubEvent* sub =
           //    dynamic_cast<hadaq::TdcSubEvent*> (ev->GetSubEvent(fTdcId));
        ...
        for (Int_t hit_no_a = 1; hit_no_a <  CHANNELS; hit_no_a++){
          ...
          for (Int_t hit_no_b = 1; hit_no_b < CHANNELS; hit_no_b++){
            ...
            //~ if(fHitsLay3[hit_no_b][3]  &&  fHitsLay2[hit_no_a][3] ){
              ...
              Float_t t1_a = (fHitsLay2[hit_no_a][1]+TRIGGERDELAY);
              Float_t t1_b = (fHitsLay3[hit_no_b][1]+TRIGGERDELAY);
 ...
 //~ // 
  //~ // t1_a > t1L && t1_a < t1R && t1_b > t1L && t1_b < t1R && 
              if (  tot_a > tot_low &&  tot_b > tot_low && tot_a < tot_high && tot_b < tot_high && t1_a>t1L &&t1_b>t1L && t1_a<t1R && t1_b<t1R ){
                FillH2(coinc_matrix,hit_no_a,hit_no_b);
                //~ if( (hit_no_a == (hit_no_b - 2) ) ) {
                //if( (hit_no_a == 4 &&  ( hit_no_b == 6 || hit_no_b == 7 )) ) { 
				if( hit_no_a == 4 &&   hit_no_b == 6 ) { 	 
                 Float_t tsum = (t1_a + t1_b) ;
                 Float_t tdiff = (t1_a - t1_b) ;
                 FillH2(meta_fish,tsum,tdiff );
                 FillH2(potato_fish_h,t1_a-200,tot_a );
                 FillH2(potato_fish_h,t1_b+200,tot_b );
                 if(fabs(tdiff) <5 ) FillH1(fish_width_h,tsum );
                 if(fabs(tdiff) <5 ) fish_width_h2->Fill(tsum );
         ...
         //TF1 *func = new TF1("gaus",fitf,-200,200,3);
         fish_width_h2->Fit("gaus","Q","",-100,100);
...
class SecondProcLgad : public base::EventProc {
   protected:
...
//       int evt_no;
   ...
   public:
      SecondProcLgad(const char* procname, const char* _tdcid1, const char* _tdcid2) :
         base::EventProc(procname),
         fTdcId1(_tdcid1),
         fTdcId2(_tdcid2),
         hNumHits(0)
      {
		...
		//gStyle->SetOptFit(1);
        Int_t nchan_coinc = 18;
        coinc_matrix = MakeH2("coinc_matrix","coinc_matrix", nchan_coinc, 0.5, 0.5+nchan_coinc,  nchan_coinc, 0.5, 0.5+nchan_coinc,"channel layer2; channel layer 3" );
        meta_fish =  MakeH2("meta_fish","meta_fish", 500, -200, +300, 200, -100, 100, "t1a + t1b (ns); t1a - t1b  (ns)");
        potato_fish_h = MakeH2("potato_fish","potato_fish",1000,-200, +200,1000, 0, 1000, "t1DIFF(LGAD-MDC) (ns);totMDC (ns)");
        fish_width_h =  MakeH1("fish_width","fish_width", 200, -200, +300,  "t1a + t1b (ns) ");
        tdiff_h =  MakeH1("tdiff","tdiff", 5000, -2000, +3000,  "t1LGAD - t1MDC (ns) ");    
            
        fish_width_h2 =  new TH1F("tdiff_width2","tdiff_width2", 5000, -2000, +3000  );
        fish_width_h2->SetDirectory(0);
        RegisterObject(fish_width_h2);
        ...
        for (Int_t hit_no_a = 1; hit_no_a <  CHANNELS; hit_no_a++){
          ...
          for (Int_t hit_no_b = 1; hit_no_b < CHANNELS; hit_no_b++){
            ...
            //~ if(fHitsLay3[hit_no_b][3]  &&  fHitsLay2[hit_no_a][3] ){
              ...
              Float_t tdiff = (t1_a - t1_b) ;
              if( hit_no_a == 8 &&   hit_no_b == 4 ){
					FillH1(tdiff_h,tdiff );
					fish_width_h2->Fill(tdiff );
					FillH2(potato_fish_h,tdiff,tot_b );
				}
				if(fabs(tdiff-TRIGGERDELAY)< 100) 	FillH2(coinc_matrix,hit_no_a,hit_no_b);
  //~ // t1_a > t1L && t1_a < t1R && t1_b > t1L && t1_b < t1R && 
              if (  tot_a > tot_low &&  tot_b > tot_low && tot_a < tot_high && tot_b < tot_high && t1_a>t1L &&t1_b>t1L && t1_a<t1R && t1_b<t1R ){
         ...
         //TF1 *func = new TF1("gaus",fitf,-200,200,3);
         fish_width_h2->Fit("gaus","Q","",-300,305);
...
//   tree_out->Close();
 ...
 new SecondProc("Sec_0350", "TDC_0350");
 //~ new SecondProc("Sec_1800", "TDC_1800");
 //~ new SecondProc("Sec_1802", "TDC_1802");
 new SecondProc("Sec_1803", "TDC_1803");
 new SecondProc("Sec_1806", "TDC_1806");
 new SecondProc("Sec_1807", "TDC_1807");
 //~ new SecondProc("Sec_1808", "TDC_1808");
 //~ new SecondProc("Sec_1809", "TDC_1809");
 //~ new SecondProc("Sec_1810", "TDC_1810");
 //~ new SecondProc("Sec_1811", "TDC_1811");
 new SecondProc("Sec_1812", "TDC_1812");
 new SecondProc("Sec_1813", "TDC_1813");
 //~ new SecondProc("Sec_1814", "TDC_1814");
 //~ new SecondProc("Sec_1815", "TDC_1815");
 new SecondProc("Sec_1816", "TDC_1816");
 new SecondProc("Sec_1817", "TDC_1817");
 new SecondProc("Sec_1820", "TDC_1820");
 new SecondProc("Sec_1821", "TDC_1821");


 new SecondProcFish( "Sec_fish", "TDC_1807", "TDC_1812");
 new SecondProcLgad( "Sec_Lgad",  "TDC_0350", "TDC_1807");
}
