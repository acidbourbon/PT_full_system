...
//       base::H1handle  totCh2; //!< histogram with hits number
      	double      fHitsLay2[33][4];    //!< 17 channel, abstract hits, two dges
		double      fHitsLay3[33][4];    //!< 17 channel, abstract hits, two dges
		double      fHits[53][4];    //!< 17 channel, abstract hits, two dges
...
long Unpack(hadaq::TdcSubEventFloat* sub, double hits[][4]) 
    {
        long msgcnt = 0;
...
//          printf("### DEBUG ###\n");
		...
		num =  Unpack(sub, 0, fHits);
        num1 = Unpack(subLay2,  fHitsLay2);
        num2 = Unpack(subLay3,  fHitsLay3);     
