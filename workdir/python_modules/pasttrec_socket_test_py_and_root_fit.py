...
def scurve_scan(serial_no):
    ...
    for i in range(0,8): 
        ...
        py_noise_fit_chi2 += [ TS / float(NDF) ]
          # plt.show()
        pdf.savefig(fig00)
    pdf.close()
    ...
    if os.path.isfile(root_name):
        ...
        for i in range(0,0+n_channels):
          ...
          ndata = dummy_tsbl_tree.Draw("dummy_calib_tsbl_scan_raw>>raw_scan(60,3,63)","name == " + str(name) + " && channels == " + str(i) )
    ...
    for i in range(0,8):
        ...
        if baseline_mean[i] < 15 and baseline_mean[i] > -15 and noise_sigma[i] < 4.0 and noise_sigma[i] > 0 and noise_halfmax[i] < 40 and noise_halfmax[i] > 3  and py_noise_fit_chi2[i] < 5 :
            passed_test += ['OK']
        else:
            passed_test += ['no baseline']
            
    return baseline_mean, noise_halfmax, py_noise_halfmax, noise_sigma, py_noise_sigma, py_noise_fit_chi2,  passed_test
# In[ ]:
