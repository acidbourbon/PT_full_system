...
def scurve_scan(serial_no):
    ...
    for i in range(0,8): 
      ...
      fig00 = plt.figure(num=None, figsize=(15, 10), dpi=80, facecolor='w', edgecolor='k')
      plt.scatter(tsbl_range,nums,alpha=1,label = "{:d}".format(i))
    ...
    for i in range(0,8):
        ...
        if baseline_mean[i] < 15 and baseline_mean[i] > -15 and noise_sigma[i] < 4.0 and noise_sigma[i] > 0 and noise_halfmax[i] < 40 and noise_halfmax[i] > 3  :
            passed_test += ['OK']
        else:
            passed_test += ['no baseline']
            
    return baseline_mean, noise_halfmax, noise_sigma, passed_test
# In[ ]:
