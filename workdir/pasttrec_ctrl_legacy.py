...
def set_baseline( TDC, channel,val):
  ...
  chip_chan=channel%8
  os.system("cd pasttrec_ctrl_legacy; TDC="+TDC+" CONN={:d} CHIP={:d} ./baseline {:d} {:d}".format(conn,chip, chip_chan ,val))

  return

def set_threshold(TDC,conn,chip,thresh):
  os.system("cd pasttrec_ctrl_legacy; TDC="+TDC+" CONN={:d} CHIP={:d} ./threshold {:d}".format(conn,chip,thresh))
...
def init_chip(TDC,conn,chip,pktime,gain,thresh):
  ...
  if( pktime == 10 ):
    os.system("cd pasttrec_ctrl_legacy; TDC="+TDC+" CONN={:d} CHIP={:d} ./spi blue_settings_pt10_g1_thr127".format(conn,chip))
  if( pktime == 15 ):
    os.system("cd pasttrec_ctrl_legacy; TDC="+TDC+" CONN={:d} CHIP={:d} ./spi black_settings_pt15_g1_thr127".format(conn,chip))
  if( pktime == 20 ):
    os.system("cd pasttrec_ctrl_legacy; TDC="+TDC+" CONN={:d} CHIP={:d} ./spi black_settings_pt20_g1_thr127".format(conn,chip))

  os.system("cd pasttrec_ctrl_legacy; TDC="+TDC+" CONN={:d} CHIP={:d} pktime={:d} gain={:d} ./set_gain_pktime".format(conn,chip,pktime,gain))
  os.system("cd pasttrec_ctrl_legacy; TDC="+TDC+" CONN={:d} CHIP={:d} ./threshold {:d}".format(conn,chip,thresh))
...
def reset_board(TDC,conn):
  os.system("cd pasttrec_ctrl_legacy; TDC="+TDC+" CONN={:d} ./reset ".format(conn))
